...
The nested analysis selected no gene in \NestedNoGeneFolds\ of 50 training folds; \NestedOutlierFallbacks\ folds required a no-replacement retry after a DESeq2 outlier-replacement error. The mean out-of-fold concordance increment was \NestedDelta\ (patient-bootstrap 95\% interval \NestedCiLow\ to \NestedCiHigh); the mean gene-minus-clinical three-year Brier-score difference was \NestedBrierDelta. The predeclared prediction criterion \NestedPassed. The separate clinical-only null simulations are reported in the committed result table and do not change that criterion.
