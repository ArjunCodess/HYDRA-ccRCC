...
541 tumor aliquots and 533 patients differ by 8 aliquots. Those 8 are the extra samples from four patients with three tumor aliquots each. The 533 patients include 175 deaths. The primary Cox model and the nested splits require positive overall-survival time plus age, sex, stage, and grade, which leaves 517 patients and 170 deaths. The other 16 fail exactly one field: 8 lack grade, 3 lack stage, 1 lacks age, and 4 have survival time recorded as zero. Five of those 16 are deaths. The stage-only sensitivity restores the 8 without grade (525 patients). The grade-only sensitivity restores the 3 without stage (520 patients). Counts are in `results/tables/nested_cohort_exclusions.csv` and `results/figures/cohort_flow.png`. Normal aliquots are already one per patient: 72 normals, 72 patients, no patient with more than one normal. The 72 paired DESeq2 patients are those with both a selected tumor and a selected normal. The paired model is `~ patient + condition`.
...
The ridge arm is a training-fold glmnet Cox model. Clinical covariates are unpenalized. Every training-fold reproducible gene with a nonzero training standard deviation gets an L2 penalty (`alpha = 0`), so the fit does not select a sparse subset and does not freeze a signature. Before tuning, the outer training fold fixes DESeq2 size factors, test size factors from the training geometric-mean reference, expression as log2(count / size factor + 1), and the nonzero training-SD gene filter. `cv.glmnet` then fits the lambda path on all outer-training rows and scores that path by partial-likelihood deviance on five event-stratified inner folds of those training patients. `lambda.min` is that deviance minimum, not the C-index and not the one-standard-error rule. Each inner fit standardizes its own training rows. The coefficients applied to the outer test rows come from the refit on the full outer training fold, and those outer-training moments are applied to the test rows. The inner-fold seed is `RESAMPLING$seed + 31000 + repeat_id * 10 + fold`. Test outcomes are not an input. This arm is not the acceptance test.
