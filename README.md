...
[The candidate evidence table](results/tables/high_confidence_candidate_evidence.csv) and [interpretation context](results/tables/candidate_interpretation_context.csv) list genes alphabetically. Earlier manual “lead” tiers and a weighted shortlist score were removed because they did not incorporate the contradictory external and composition findings. The [prior 24-gene biological review](results/archive/legacy_biological_review_24_candidates.md) is retained only as a superseded historical record.
...
Allow several gigabytes of free disk space during regeneration: the pipeline writes a temporary DESeq2 RDS before replacing its existing cache, and a full rerun needs room for both copies.
